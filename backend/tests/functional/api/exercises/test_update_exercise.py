...
def test_updates_exercise(authenticated_api_client, unit_of_work):
    ...
    update_response = authenticated_api_client.put(
        f"/exercises/{exercise.id}", json=updated_exercise
    )

    assert update_response.status_code == 204
    assert update_response.content == b""

    get_response = authenticated_api_client.get(f"/exercises/{exercise.id}")
    assert get_response.status_code == 200
    assert get_response.json() == {"id": exercise.id, **updated_exercise}


def test_update_response_not_found_when_exercise_does_not_exist(
    ...
    response = authenticated_api_client.put("/exercises/123", json=updated_exercise)

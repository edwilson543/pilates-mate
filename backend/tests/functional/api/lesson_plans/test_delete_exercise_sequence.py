...
def test_deletes_existing_exercise_sequence(unit_of_work, authenticated_api_client):
    ...
    response = authenticated_api_client.delete(
        f"/lesson-plans/sequences/{sequence_1.id}"
    )
...
def test_response_not_found_when_deleting_nonexistent_sequence(
    authenticated_api_client,
):
    response = authenticated_api_client.delete("/lesson-plans/sequences/999")
